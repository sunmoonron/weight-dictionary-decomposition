"""R4: the CONTINUATION LENS across architectures and on the tuned lens's own corpus.
...
VARIANTS = ["logit", "mlp", "sink", "actual"] + (["tuned"] if tuned else [])
acc = {}
...
tag = MODEL.split("/")[-1]
with open(f"{OUT}/exp_r4_{tag}_{DATA}.json", "w") as f:
