"""Experiment J RERUN -- transformation vs erasure, with the two audit fixes.

Fixes vs exp_j_transform.py (which crashed before json.dump and had a leak):
  1. W_U is moved to CPU before the qualitative unembedding section
     (original: cuda W_U @ cpu dirs -> device-mismatch crash at the end).
  2. States are centered on the TRAIN-HALF mean only (original centered on the
     full corpus, leaking the test-half mean into a no-intercept ridge probe).
Everything else mirrors the original. Output: results/exp_j_rerun.json.
"""
...
NT = levels[7].shape[0]
split = NT // 2                                    # FIX 2: define split first,
mu = {lv: x[:split].mean(0) for lv, x in levels.items()}
levels = {lv: x - mu[lv] for lv, x in levels.items()}  # center on TRAIN mean only
X6 = levels[7]                                     # layer-6 readout (post block 6)
print(f"{NT} states captured (train-half centering)", flush=True)
...
res = {"note": "rerun with W_U.cpu() fix and train-half-only centering",
       "groups": {}}
...
W_U = model.transformer.wte.weight.detach().float().cpu()   # FIX 1: .cpu()
...
with open(f"{OUT}/exp_j_rerun.json", "w") as f:
    json.dump(res, f, indent=1)
print("DONE_J_RERUN", flush=True)
