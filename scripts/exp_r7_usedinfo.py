...
Writes results/exp_r7_usedinfo.json
"""
...
for gname, idx in sel.items():
    ...
    per_case[gname] = rows
    keys = ["E_src_full", "E_src_mlp", "E_src_sink", "E_dir_full", "E_dir_mlp", "E_dirall_full", "E_rand_full", "state_shift"]
    ...
    print(f"{gname:17s} n={len(rows)}  E_src_full med {summ['E_src_full']['median']:.4f}  E_src_mlp {summ['E_src_mlp']['median']:.4f}  "
          ...
          f"E_rand {summ['E_rand_full']['median']:.4f}  dir/src {summ['dir_over_src_median']:.2f}  shift {summ['state_shift']['median']:.3f}  {time.time() - t0:.0f}s", flush=True)

G = res["groups"]
res["pre_registered"] = {
    "U1_erased_src_ge_half_intact": G["erased_transient"]["E_src_full"]["median"] >= 0.5 * G["intact"]["E_src_full"]["median"],
    "U1_ratio": G["erased_transient"]["E_src_full"]["median"] / max(G["intact"]["E_src_full"]["median"], 1e-9),
    "U2_erased_dir_lt_0.1_src": G["erased_transient"]["E_dir_full"]["median"] < 0.1 * G["erased_transient"]["E_src_full"]["median"],
    "U3_intact_dir_over_src_ge_0.5": G["intact"]["dir_over_src_median"] >= 0.5}
res["per_case"] = per_case
res["elapsed_s"] = time.time() - t0
with open(f"{OUT}/exp_r7_usedinfo.json", "w") as f:
