"""R6b: the self-trained tuned lens (exp_r6, trained on the evaluation corpus) vs the window continuations.
Adds variant `self` = h + A h + b with params from results/exp_r6_lens_{tag}_{data}.pt.
Derived from R5: continuation lens with SMALL LOCAL WINDOWS -- is the trained lens's remaining edge (levels 1-3 on the Pile) local context?
...
SELF = None
_p = f"{OUT}/exp_r6_lens_{MODEL.split('/')[-1]}_{DATA}.pt"
if os.path.exists(_p):
    _sd = torch.load(_p, map_location=DEV, weights_only=True)
    SELF = {i: (_sd[f"{i}.weight"].float(), _sd[f"{i}.bias"].float()) for i in range(NL)}
    print(f"self-trained lens loaded from {_p}", flush=True)
else:
    print(f"NO self-trained lens at {_p}", flush=True)
VARIANTS = ["logit", "mlp", "sink", "sinkprev", "win4", "actual"] + (["tuned"] if tuned else []) + (["self"] if SELF else [])
acc = {}
...
tag = MODEL.split("/")[-1]
with open(f"{OUT}/exp_r6b_{tag}_{DATA}.json", "w") as f:
...
for v in ["mlp", "sink", "sinkprev", "win4"]:
    if tuned:
        print(f"{v} beats released-tuned at {sum(M[f'{l}|{v}']['kl'] < M[f'{l}|tuned']['kl'] for l in range(NL))}/{NL}")
    if SELF:
        print(f"{v} beats SELF-trained at {sum(M[f'{l}|{v}']['kl'] < M[f'{l}|self']['kl'] for l in range(NL))}/{NL}")
if SELF and tuned:
    print(f"self-trained beats released at {sum(M[f'{l}|self']['kl'] < M[f'{l}|tuned']['kl'] for l in range(NL))}/{NL}")
