...
def mk_group_hook(m):
    def f(mod, inp, out):
        a = state["active"]
        if a and a[0] == "group" and a[2] == m and not state.get("busy"):
            state["busy"] = True
            try:
                blk = blocks[m]
                x = blk.ln_2(state["stash"][a[1]])
                z_wb = torch.addmm(mod.bias, x.reshape(-1, x.shape[-1]), mod.weight).view(out.shape)
            finally:
                state["busy"] = False
            idx = state["group"]
            out = out.clone()
...
res = json.load(open(f"{OUT}/exp_r2_depends.json"))
assert len(res["mlp_edge"]) == 78 and len(res["attn_edge"]) == 66, "layer-level grid incomplete"
print(f"loaded layer-level grid; clean CE in file {res['clean_ce']:.4f} vs now {clean_ce:.4f}", flush=True)
# group sanity: early-reading ALL neurons from level l must equal the MLP-edge cell
s = run(("group", 6, 8), group=torch.arange(DFF, device=DEV))
print("sanity group(all,6,8) vs mlp_edge(6,8):", s["dce"], res["mlp_edge"]["6|8"]["dce"], flush=True)
res["sanity_group_all_vs_edge"] = {"group_all": s, "edge": res["mlp_edge"]["6|8"]}
